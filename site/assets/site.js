(async function(){
  ...
  function enhanceMobileNavigation(){
    ...
    btn.onclick=()=>{sheet.hidden=false;btn.setAttribute('aria-expanded','true');document.body.classList.add('nav-open');sheet.querySelector('.mobile-nav-close').focus()};sheet.querySelector('.mobile-nav-close').onclick=()=>{close();btn.focus()};sheet.addEventListener('click',e=>{if(e.target===sheet)close()});sheet.addEventListener('keydown',e=>{if(e.key==='Escape'){close();btn.focus()}});sheet.querySelectorAll('a').forEach(a=>a.addEventListener('click',close));
  }
